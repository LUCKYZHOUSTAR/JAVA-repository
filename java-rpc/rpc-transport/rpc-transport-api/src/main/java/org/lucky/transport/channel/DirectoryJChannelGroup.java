...
public class DirectoryJChannelGroup {
     ...
     * 获取指定group的引用计数
     */
    public int getRefCount(LChannelGroup group) {
     ...
     * 指定group的引用计数 +1
     */
    public int incrementRefCount(LChannelGroup group) {
     ...
     */
    public int decrementRefCount(LChannelGroup group) {
        AtomicInteger counter = groupRefCounter.get(group);
        if (counter == null) {
            return 0;
        }
    ...
    static class GroupRefCounterMap extends ConcurrentHashMap<LChannelGroup, AtomicInteger> {

        private static final long serialVersionUID = 6590976614405397299L;

        public AtomicInteger getOrCreate(LChannelGroup key) {
