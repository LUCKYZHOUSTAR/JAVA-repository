 ...
 * @author jiachun.fjc
 */
public class NettyChannelGroup implements LChannelGroup {
    ...
    public LChannel next() {
        for (; ; ) {
            ...
            if (length == 1) {
                return (LChannel) elements[0];
            }

            int index = indexUpdater.getAndIncrement(this) & Integer.MAX_VALUE;

            return (LChannel) elements[index % length];
        }
    }

    @Override
    public List<? extends LChannel> channels() {
    ...
    @Override
    public boolean add(LChannel channel) {
    ...
    @Override
    public boolean remove(LChannel channel) {
