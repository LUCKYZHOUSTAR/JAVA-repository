...
import org.lucky.transport.netty.channel.NettyChannelGroup;
...
public abstract class NettyConnector implements JConnector<JConnection> {
    ...
    //一个connecor对应多个提供方也就是多个jchanenlgroup
    //一个提供方，对应一个jchannelgroup，这样看来一个connector相当于把所有的提供方都包含了
    //所以dfaultclient包含了对connector的引用
    private final ConcurrentMap<UnresolvedAddress, LChannelGroup> addressGroups = Maps.newConcurrentMap();
    ...
    @Override
    public LChannelGroup group(UnresolvedAddress address) {
        checkNotNull(address, "address");

        LChannelGroup group = addressGroups.get(address);
        if (group == null) {
            LChannelGroup newGroup = channelGroup(address);
    ...
    @Override
    public Collection<LChannelGroup> groups() {
        return addressGroups.values();
    }

    @Override
    public boolean addChannelGroup(Directory directory, LChannelGroup group) {
    ...
    @Override
    public boolean removeChannelGroup(Directory directory, LChannelGroup group) {
        CopyOnWriteGroupList groups = directory(directory);
    ...
    public boolean isDirectoryAvailable(Directory directory) {
        CopyOnWriteGroupList groups = directory(directory);
        LChannelGroup[] snapshot = groups.snapshot();
        for (LChannelGroup g : snapshot) {
            if (g.isAvailable()) {
                return true;
            }
        }
     ...
     */
    protected LChannelGroup channelGroup(UnresolvedAddress address) {
